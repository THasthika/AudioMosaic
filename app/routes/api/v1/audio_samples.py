...
from app.schemas.audio_sample_label import AudioSampleLabelCreateRequest, AudioSampleLabelCreate, AudioSampleLabelUpdate
...
async def get_audio_sample_by_id(id: UUID, db: Annotated[Session, Depends(get_db)]):
...
async def accept_audio_sample(id: UUID, db: Annotated[Session, Depends(get_db)]):
...
async def reject_audio_sample(id: UUID, db: Annotated[Session, Depends(get_db)]):
    result = AudioSampleService(db).update_approval_status(
...
async def delete_audio_sample(
    id: UUID, background_tasks: BackgroundTasks, db: Annotated[Session, Depends(get_db)]
):
...
async def get_audio_sample_data(id: UUID, db: Annotated[Session, Depends(get_db)]):
...
@router.post("/sample/{id}/labels", status_code=status.HTTP_201_CREATED, tags=["Audio Samples"])
async def create_audio_sample_label(id: UUID,
                                    db: Annotated[Session, Depends(get_db)],
                                    request_body: AudioSampleLabelCreateRequest):

    create_model = AudioSampleLabelCreate(is_sample_level=request_body.is_sample_level,
                                          start_time=request_body.start_time,
                                          end_time=request_body.end_time,
                                          label_id=request_body.label_id,
                                          audio_sample_id=id)
    result = AudioSampleLabelService(
        db).create_audio_sample_label(create_model)
    return handle_result(result)


@router.delete("/sample/{id}/labels/{instance_id}", tags=["Audio Samples"])
async def delete_audio_sample_label(id: UUID, instance_id: UUID, db: Annotated[Session, Depends(get_db)]):

    result = AudioSampleLabelService(
        db).delete_audio_sample_label(id, instance_id)
    return handle_result(result)


@router.patch("/sample/{id}/labels/{instance_id}", tags=["Audio Samples"])
async def update_audio_sample_label(id: UUID,
                                    instance_id: UUID,
                                    db: Annotated[Session, Depends(get_db)],
                                    request_body: AudioSampleLabelUpdate):

    result = AudioSampleLabelService(
        db).update_audio_sample_label(id, instance_id, request_body)
    return handle_result(result)


@router.get("/sample/{id}/labels", tags=["Audio Samples"])
async def get_audio_sample_labels(id: UUID, db: Annotated[Session, Depends(get_db)]):
