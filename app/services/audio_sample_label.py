...
from app.schemas.audio_sample_label import AudioSampleLabelCreate, AudioSampleLabelItem, AudioSampleLabelUpdate
...
class AudioSampleLabelService(BaseService):
    ...
    def _get_sample_label(self, audio_sample_id: UUID, label_instance_id: UUID) -> AudioSampleLabel:
        audio_sample_label: AudioSampleLabel = AudioSampleLabelRepository(
            self.db).get_by_id(label_instance_id)

        if audio_sample_label is None:
            raise AppExceptionCase(status.HTTP_404_NOT_FOUND, {
    ...
    def create_audio_sample_label(self, create_request: AudioSampleLabelCreate) -> ServiceResult:

        try:
            create_request = AudioSampleLabelService._validate_audio_sample_label_times(
                create_request)

            created_model: AudioSampleLabel = AudioSampleLabelRepository(
                self.db).create(create_request)
            return ServiceResult(AudioSampleLabelItem.from_orm(created_model))
        except AppExceptionCase as e:
            return ServiceResult(e)
        except IntegrityError as e:
            print(e)
            return ServiceResult(AppExceptionCase(
                status.HTTP_400_BAD_REQUEST, {"reason": "could not create"}))
        except Exception as e:
            print(e)
            return ServiceResult(AppExceptionCase(
                status.HTTP_500_INTERNAL_SERVER_ERROR, {"reason": "something went wrong"}))

    def delete_audio_sample_label(self, audio_sample_id: UUID, label_instance_id: UUID) -> ServiceResult:
        ...
        try:
            ...
            return ServiceResult(AudioSampleLabelItem.from_orm(audio_sample_label))
        except AppExceptionCase as e:
            return ServiceResult(e)
        except Exception as e:
            print(e)
    ...
    def update_audio_sample_label(self,
                                  audio_sample_id: UUID,
                                  label_instance_id: UUID,
                                  update_request: AudioSampleLabelUpdate) -> ServiceResult:
        try:
            update_request = AudioSampleLabelService._validate_audio_sample_label_times(
                update_request)

            audio_sample_label = self._get_sample_label(
                audio_sample_id, label_instance_id)
            updated_model = AudioSampleLabelRepository(
                self.db).update(audio_sample_label.id, update_request)
    ...
    def get_audio_sample_labels(self, audio_sample_id: UUID) -> ServiceResult:
        try:
            audio_sample_labels = AudioSampleLabelRepository(
                self.db).get_by_audio_sample_id(audio_sample_id)
            return ServiceResult(list(map(lambda x: AudioSampleLabelItem.from_orm(x), audio_sample_labels)))
        except Exception as e:
            print(e)
            return ServiceResult(AppExceptionCase(
                status.HTTP_500_INTERNAL_SERVER_ERROR, {"reason": "something went wrong"}))
