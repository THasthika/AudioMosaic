...
class AudioSampleService(BaseService):
    ...
    def _make_insert_models(self, dataset_id: UUID, audio_sample_files: list[UploadFile]) -> list[AudioSampleCreate]:
    ...
    async def batch_upload_audio_samples(self, dataset_id: UUID, audio_sample_files: list[UploadFile]) -> ServiceResult:
        ...
        try:
            ...
            # validate files
            for file in audio_sample_files:
                if not AudioSampleService._is_audio_sample_valid_content_type(file):
                    raise audio_sample_exceptions.AudioSampleIncorrectContentType(
